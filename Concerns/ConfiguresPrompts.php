...
use Voyager\Console\PromptValidationException;
...
trait ConfiguresPrompts
{
    ...
    protected function configurePrompts(InputInterface $input)
    {
        ...
        Prompt::interactive(($input->isInteractive() && defined('STDIN') && stream_isatty(STDIN)) || $this->venusian->runningUnitTests());

        Prompt::validateUsing(fn (Prompt $prompt) => $this->validatePrompt($prompt->value(), $prompt->validate));

        Prompt::fallbackWhen(windows_os() || $this->venusian->runningUnitTests());
    ...
    protected function promptUntilValid($prompt, $required, $validate)
    {
        while (true) {
            ...
            if ($required && ($result === '' || $result === [] || $result === false)) {
                ...
                if ($this->venusian->runningUnitTests()) {
    ...
    {
        return $this->venusian['validator']->make(
            [$field => $value],
            [$field => $rules],
    ...
    private function multiselectFallback($label, $options, $default = [], $required = false)
    {
        ...
        if ($required === false && ! $this->venusian->runningUnitTests()) {
