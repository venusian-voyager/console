...
class ScheduleListCommand extends Command
{
     ...
     */
    private function getCronExpressionSpacing($events, DateTimeZone $timezone): array
    {
     ...
     * @return array
     */
    private function listEvent($event, $terminalWidth, $expressionSpacing, $repeatExpressionSpacing, $timezone, $convertedExpression = null): array
    {
        ...
        $command = preg_replace("#(php computer [\w\-:]+) (.+)#", '$1 <fg=yellow;options=bold>$2</>', $command);
     ...
     */
    private function sortEvents(\Voyager\NutsAndBolts\Collection $events, DateTimeZone $timezone): Collection
    {
     ...
     *
     * @param  \Voyager\Console\Scheduling\CallbackEvent  $event
     * @return string
     */
    private function getClosureLocation(CallbackEvent $event): string
    {
        ...
        if ($callback instanceof Closure) {
            ...
            return sprintf(
                '%s:%s',
                str_replace($this->venusian->basePath().DIRECTORY_SEPARATOR, '', $function->getFileName() ?: ''),
